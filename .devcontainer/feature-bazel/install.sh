...
# renovate: datasource=github-tags depName=bazel-contrib/multitool
MULTITOOL_VERSION="v0.9.0"
...
if [[ $ARCH == "arm64" ]] || [[ $ARCH == "aarch64" ]]; then
    ...
    curl "https://github.com/bazel-contrib/multitool/releases/download/${MULTITOOL_VERSION}/multitool-aarch64-unknown-linux-gnu.tar.xz" -L |
        tar -xJOf - --strip-components=1 multitool-aarch64-unknown-linux-gnu/multitool > /usr/local/bin/multitool
    ...
    curl "https://github.com/bazel-contrib/multitool/releases/download/${MULTITOOL_VERSION}/multitool-x86_64-unknown-linux-gnu.tar.xz" -L |
        tar -xJOf - --strip-components=1 multitool-x86_64-unknown-linux-gnu/multitool > /usr/local/bin/multitool
else
